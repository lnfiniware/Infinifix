...
def plan(ctx, detected: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if intel_audio and audio_broken:
        actions.append(
            {
                "id": "set_dsp_driver_3",
        ...
        if not detected.get("grub_param_present"):
            actions.append(
                {
                    "id": "add_grub_dsp_param",
                    "description": "append kernel param snd_intel_dspcfg.dsp_driver=3 in /etc/default/grub",
                    "safe": True,
                    "advanced": False,
                }
            )
...
def verify(ctx, detected: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ok = "card " in aplay.stdout.lower() and pactl.returncode == 0
    return {
        "ok": ok,
        "message": "Audio stack looks healthy" if ok else "Audio still needs manual check",
    }
